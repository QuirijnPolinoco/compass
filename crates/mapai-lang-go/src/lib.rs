//! `mapai-lang-go` — the Go language extractor.
//!
//! A self-contained unit behind the [`mapai_extract::Extractor`] trait: detection, the
//! tree-sitter-go grammar, per-file symbol/import extraction, and Go's whole-repo import
//! resolution. It depends only on `mapai-extract` + `mapai-core` (ADR-0002).
