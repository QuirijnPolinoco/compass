//! `mapai-extract` — the STABLE contract every language plugs into.
//!
...
use mapai_core::{FileId, LanguageId, Span, SymbolKind};
...
/// Opaque per-language configuration carrier (e.g. derived from `.mapai.toml`).
