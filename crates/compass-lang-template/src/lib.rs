//! TEMPLATE for a new Compass language extractor.
//!
//! Copy this crate to `crates/compass-lang-<name>/`, rename it, and work through the TODOs
//! below. The whole job is implementing one trait — [`compass_extract::Extractor`] — so a
//! new language never touches `compass-core`, `compass-engine`, or `compass-mcp` (the North
//! Star, ADR-0002). See `CONTRIBUTING.md` §4 for the full Definition of Done, and the
//! existing `compass-lang-go` / `compass-lang-python` crates for worked examples.
...
// TODO: add tests with fixtures inside this crate (see compass-lang-go/src/lib.rs `tests`).
// `cargo test -p compass-lang-<name>` should pass before you open a PR.
