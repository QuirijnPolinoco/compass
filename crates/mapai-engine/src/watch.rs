...
//!
//! Build/VCS/cache directories are ignored so that writing our own `.mapai/` cache can't
...
const IGNORED_DIRS: [&str; 3] = [".git", ".mapai", "target"];
...
mod tests {
    ...
    fn ignores_cache_vcs_and_build_paths() {
        assert!(is_ignored(Path::new("repo/.mapai/graph.json")));
        assert!(is_ignored(Path::new("repo/.git/HEAD")));
        assert!(is_ignored(Path::new("target/debug/mapai")));
