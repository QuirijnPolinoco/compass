...
| `Parsing` | `compass-extract` | same | How an extractor reads a file: `Grammar` (tree-sitter + `extract`, the default every language uses) or `Head { max_bytes }` (only the top of the file, handed to `extract_head` — a CSV header; exempt from the size cap because the body is never read) |
| `FileCategory` | `compass-core` | same | An open string on every file (`code`, `markup`, `data`, …), declared by its extractor (`Extractor::category`, defaulted to `code`). Core reads one bit — code-like or *supporting*: every dependency metric (impact/guard, cycles, hubs, degree, communities, isolated files, context seeding) uses only edges between code-like files, while navigation queries and the map see everything (ADR-0007) |
| `Calls` edge | `compass-core` | same | Symbol → Symbol; resolved by the engine from each extractor's raw calls (same-file match, else a unique one within the caller's call namespace — its language, shared by C/C++ and by Java/Kotlin; ambiguous skipped). Emitted by every language extractor, each naming only the callees it can identify without type information (plain calls, calls on `this`/`self`, constructors) |
