...
| Serialization / cache | `serde` on core types; versioned on-disk format under `.mapai/` | [ADR-0004](decisions/0004-serde-placement-and-cache-format-versioning.md) |
...
MapAI sits between a **codebase on disk** and the people/tools that need to understand it.
A **developer** runs it from the CLI to see an overview; an **AI assistant** (any
MCP-capable host — Claude, Gemini, ChatGPT, etc.) connects to MapAI's MCP server to query
the map. MapAI reads the repository (respecting `.gitignore`) and uses the **tree-sitter**
...
`mapai-core`. Only the CLI composition root depends on language crates (gated by cargo
...
| `mapai-lang-template` | Copy-paste skeleton for a new language; **excluded** from the workspace so it never links or gates CI | (template only) |

**Why the seams are here.** The durable boundary is `mapai-extract` — the contract between
...
`mapai-extract` + `mapai-core`. `mapai-core` stays a **pure domain model** — no MCP, no
tree-sitter, no language, no walk/notify/rayon — because it is the universal dependency sink
that everything else serializes and queries. `mapai-core` and `mapai-extract` are kept
...
**modules** inside `mapai-engine`; a module earns its own crate only on a documented trigger
...
form live in `mapai-core` (ADR-0004).

| Entity | Owned by | Stored in | Notes |
|--------|----------|-----------|-------|
| `File` node | `mapai-core` | in-memory graph + `.mapai/` cache | path (repo-relative), `LanguageId`, content hash (for incremental staleness) |
| `Symbol` node | `mapai-core` | same | name, kind (function/class/method/…), defining file, source span |
| `Imports` edge | `mapai-core` | same | File → File, produced by the `resolve` phase; unresolved targets become `Diagnostic`s (FR-12/D2) |
| `Defines` edge | `mapai-core` | same | File → Symbol |
| `Calls` edge | `mapai-core` | same | Symbol → Symbol |
| `References` edge | `mapai-core` | same | File → File; reserved for HTML/CSS link/asset edges (FR-19/H4) |
| `Diagnostic` | `mapai-core` | in-memory + surfaced via MCP | non-fatal per-file issues: parse errors, unresolved imports — the universal sink |
...
mapai-cli → mapai-engine::index
  ...
  → engine::cache writes .mapai/ (versioned)
```

**Flow B — AI query over MCP.** AI host → `mapai-mcp` (stdio tool call, e.g. *get subgraph
for file X*) → the query **port** implemented by `mapai-core` → returns a small, relevant
...
Trust boundary: MapAI runs with the user's privileges and ingests **untrusted repository
...
- **Information disclosure** → there is no network path and no telemetry; the map stays on
  disk under `.mapai/`. The MCP server listens on **stdio only** (no socket), so it is
  reachable only by the local host that launched it.
- **Code execution** → MapAI **never executes** mapped code; it only parses. This is a core
...
│                                    #   members = all crates/* EXCEPT mapai-lang-template (in `exclude`)
├── Cargo.lock
├── README.md
├── CONTRIBUTING.md
...
│   ├── mapai-core/                  # domain ONLY: graph model, File/Symbol/edge types, LanguageId,
│   │                                #   Diagnostic, query engine + query port. serde lives here (versioned form).
│   ├── mapai-extract/               # the STABLE contract: Extractor trait (extract + resolve),
...
│   │                                #   Depends on mapai-core (query port) ONLY — not the engine.
│   │
│   │  ── composition root / binary ──
│   ├── mapai-cli/                   # `mapai` binary + composition root. Owns the explicit cfg-gated
...
│   ├── mapai-lang-go/               # ── Tier 1, BUILT FIRST (walking skeleton) ──
│   │   └── tests/
│   │       ├── fixtures/            # sample Go project (co-located, self-contained)
│   │       └── *.rs                 # insta snapshot tests; `cargo test -p mapai-lang-go` compiles ONE grammar
│   ├── mapai-lang-python/           # ── Tier 1, post-skeleton; template is extracted from this 2nd extractor ──
│   ├── mapai-lang-java/             # ── Tier 1, post-skeleton ──
│   ├── mapai-lang-csharp/           # ── Tier 1, post-skeleton ──
│   └── mapai-lang-typescript/       # ── Tier 1, post-skeleton (TypeScript + JavaScript) ──
│
├── crates/mapai-lang-template/      # EXCLUDED from workspace.members; created AFTER a real extractor exists.
│
└── tests/
...
A new language = a new `crates/mapai-lang-<name>/` (with its own fixtures + tests) + an
optional workspace dependency and `lang-<name>` feature in `mapai-cli` + one line in
`register_all()`. **No edits to `mapai-core`, `mapai-engine`, or `mapai-mcp`.**

## 9. Cross-cutting decisions

- **Shared-type placement (no grab-bag crate):** `LanguageId` and all node/edge/graph domain
  value types live in `mapai-core`; `mapai-extract` defines the trait + harness + `RawImport`
  + `ResolutionContext` + `Detection` + `LangConfig` + the registry, and depends on core
  (never the reverse). The cross-crate `Diagnostic` lives in `mapai-core`. A
  `mapai-common`/`mapai-types` dumping-ground crate is forbidden.
- **Config & zero-config:** works with no config; optional `.mapai.toml` at repo root
...
- **MCP tool surface (initial):** `overview`, `file_dependencies` (deps + dependents),
  ...
  cannot drift from reality (FR-14/H2). DTOs (`schemars`) own the wire schema in `mapai-mcp`.
- **Serialization (ADR-0004):** `serde` derives on `mapai-core` types; the on-disk form is a
...
- **Testing & CI:** per-language fixtures + snapshot assertions live **inside each language
  ...
  mapai-lang-<x>`) to prove the grammar builds in isolation; **(2) per-language integration**
  — build `mapai-cli` with only that one feature, run the language's fixtures through the
...
| Contributor onboarding friction (Rust + C toolchain) — threatens the North Star | High | `mapai-lang-template` (extracted from a real extractor) + CONTRIBUTING walkthrough + feature-gated builds + documented dev env; **validate by adding the 2nd language (Python) via the template and timing it** |
