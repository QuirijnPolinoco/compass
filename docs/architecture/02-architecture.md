...
| `Calls` edge | `compass-core` | same | Symbol → Symbol; resolved by the engine from each extractor's raw calls (same-file match, else a unique global one; ambiguous skipped). Emitted by the Rust and TypeScript/JS extractors today; other languages opt in per crate |
