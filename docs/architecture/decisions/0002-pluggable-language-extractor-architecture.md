...
2. **One `compass-languages` crate with a module + cargo feature per language.** Per-language
...
3. **A Cargo workspace with one crate per language**, each depending only on the stable
   `compass-extract` interface (+ `compass-core` types), behind cargo features. Rejected
...
- `compass-extract` defines the **stable `Extractor` trait** and the supporting contract
...
- Each `compass-lang-<name>` crate implements `Extractor` for one language: it declares its
  ...
  `compass-extract` and `compass-core` types **only** — never on another language crate.
- Each crate **ships its own fixtures + snapshot tests** (`crates/compass-lang-<name>/tests/`)
  so `cargo test -p compass-lang-<name>` is a self-contained unit that compiles exactly one
  grammar.
...
- **Truly isolated `cargo test -p compass-lang-<x>` and per-crate CI** — compile and test one
...
- **Negative / trade-offs accepted:** more crates to manage; one shared composition-root
  ...
  a core/MCP change.
- **Follow-ups:** ship a `compass-lang-template` skeleton (extracted from the first real
