...
Two components must serialize the graph: `mapai-engine::cache` writes it to `.mapai/` for
fast restart, and `mapai-mcp` serializes query results to JSON for the AI host. Both act on
the same `mapai-core` domain types (`File`, `Symbol`, edges). The architecture says
`mapai-core` is a pure domain model and `mapai-mcp` owns the MCP wire schema — so we must
...
1. **`serde` derives directly on `mapai-core` types.** Pragmatic and idiomatic; one
...
- **Put `serde` derives on `mapai-core` domain types** (Option 1) and treat the core
  serialized form as a **versioned compatibility surface**: the `.mapai/` cache carries a
...
- **Keep MCP wire/result types as thin `schemars`-deriving DTO wrappers in `mapai-mcp`**, so
  the MCP tool schema is owned by `mapai-mcp` and `mapai-core` stays MCP-unaware. The cache
...
boundary the architecture promises (`mapai-core` doesn't know about MCP) and lets the wire
...
- **Negative / trade-offs accepted:** core's serialized shape is now a surface we must bump
  deliberately; a thin DTO mapping layer in `mapai-mcp` to maintain.
- **Follow-ups:** define the cache-format version constant and the reindex-on-mismatch path
  in `mapai-engine::cache`; document the bump rule (any breaking change to a serialized core
